...
def get_url(condition_field, value, conn):
    query_str = f"SELECT * FROM urls WHERE ({condition_field}=%s);"
    with conn.cursor(cursor_factory=extras.DictCursor) as cursor:
...
def insert_url(url, created_at, conn):
